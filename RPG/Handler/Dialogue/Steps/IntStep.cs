...
{
    public class IntStep : DialogueStep
    {
        ...
        public override async Task<bool> ProcessStep(DiscordClient client, DiscordChannel channel, DiscordUser user)
        {
            ...
            while (true)
            {
                ...
                var messageResult = await interactivity.WaitForMessageAsync(x => x.ChannelId == channel.Id && x.Author.Id == user.Id); //message has to be sent in same channel and by the same user 
                ...
                {
                    await TryAgain(channel, $"Ypur input is not an integer");
                    continue;
                }
                ...
                if (_minValue.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Your input value {inputValue} is smaller than: {_minValue}");
                        continue;
                    }
                }
                if (_maxValue.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Your input value {inputValue} is larger than {_maxValue}");
                        continue;
                    }
                }
